...
files, reusable chunking/vector artifacts, retrieval results, and immutable
pipeline runs. All timestamps use ISO-8601 UTC text generated by the application, for example
...
workbench. The current background pipeline executes chunking, embedding, and
...
| `current_stage` | `chunking`, `embedding`, or `retrieval` while a worker is executing; otherwise null. |
...
| `chunking_duration_ms` / `embedding_duration_ms` / `retrieval_duration_ms` | Per-run time spent resolving each implemented stage. |
| `created_at`, `started_at`, `completed_at` | UTC lifecycle timestamps. |
| `duration_ms` | Total execution duration through retrieval. |
...
`completed` are written in one SQLite transaction. Any validation or storage
...
and records every stage transition. `GET /runs/{run_id}` returns the latest
state for polling. Retrieved-chunk response exposure remains a later API phase.
