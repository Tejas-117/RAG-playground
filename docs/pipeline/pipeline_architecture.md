...
- extracted text
- current structure: pages, source blocks, offsets, bounding boxes, and metadata
...
- page and source-block metadata; section path is currently null
- source metadata
...
Every strategy uses the pinned local multilingual BERT tokeniser and emits exact
slices of canonical document text. Fixed-size and recursive chunking preserve
the configured token overlap. Paragraph chunks never overlap. A versioned
32,000-character cap handles pathological long unknown tokens safely; only
those fallback chunks omit global token offsets.
...
chunk fingerprint
  = ordered document/parse IDs + chunking configuration + chunker version
    + tokeniser identity/policy + character safety limit
...
For chunk sets the backend now:
